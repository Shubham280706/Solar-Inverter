...
1. **Backend**:
   ```bash
   cd Hackamined-Sintex/backend-2
   python -m venv .venv
...
2. **Frontend**:
   ```bash
   cd Hackamined-Sintex/Frontend
   npm install
   npm run dev
   ```
3. **Local AI**:
   Set `GROQ_API_KEY` to enable AI summaries and chat. Without it, the backend still starts and uses local fallback responses.
